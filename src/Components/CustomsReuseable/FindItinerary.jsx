    ...
    <div className="grid grid-cols-12 gap-6">
        ...
        {data.map((item, index) => (
          <div key={index} className={index === activeIndex ? 'block' : 'hidden'}>
            ...
            <h1 className="flex items-center lg:text-5xl text-lg text-white mt-10">
              ...
              <p className="text-5xl block hover:underline lg:hidden" >
                {item.title}
              </p>
            </h1>
      ...
      <div className=" lg:col-span-6 col-span-12  flex lg:mx-0">
        ...
        <div className="flex flex-col lg:w-auto w-full ">
          {data.map((item, index) => (
            <div key={index} className={index === activeIndex ? 'bg-gray-900 bg-opacity-40  relative hover:bg-blue-gray-900  text-white mb-[1px] items-center flex' : 'mb-[1px] bg-opacity-40 relative hover:bg-blue-gray-900 text-white flex items-center'}>
              <img src={item.src} className=" mr-4 hidden lg:block" alt={item.title} />
              <p  className=" w-10 h-10 mr-4 block lg:hidden" >{item.mapIcon}</p>
              <div   className="flex items-center  cursor-pointer" onClick={() => handleTabClick(index)}>
              ...
              <p className="lg:text-4xl text-xl  hidden lg:block" >
                {item.title}
              </p>
            </div>
            <div>

  ...
  const RestaurantDetails = [
    {
      ...
      readMoreLink: 'https://www.discoverlosangeles.com/eat-drink/al-bea%E2%80%99s-mexican-food',
      details:
    ...
    },
    {
      ...
      LoginShare: <LoginShare />,
      title: 'ALL DAY BABY',
      locationLink: 'https://www.google.com/maps?daddr=3200+W.+Sunset+Blvd+++90026',
      ...
      <div>
        ...
        <div className={`w-full mx-auto my-10 px-4 ${className}`}>
          <h1 className="text-gray-700  space-x-1">
            The burrito, a beloved Mexican food item featuring a flour tortilla and a range of
            fillings, has origins in northern Mexico, with most people pointing toward the border
            town of Ciudad Juárez. Los Angeles is not t bound by any particular traditions, so
            you’ll find a wide variety of burritos, including numerous veg-friendly options. The
            city is most famous burrito may be at El Tepeyac Café in Boyle Heights, but there are
            many more options besides the ginormous Manuel is Special Burrito. From classics to new
            school variations, read on for the best burritos in LA.
          </h1>
        </div>

        {RestaurantDetails.map((item, key) => (
          <div key={key}>
            <img src={item.src} className="w-full h-[600px] object-cover" alt="" />

            <div>{item.LoginShare}</div>

            <div className={`${className}`}>
              <div className="px-4">
                <h1 className="font-bold lg:text-4xl text-xl my-4">{item.title}</h1>
